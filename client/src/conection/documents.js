const BACKEND_URL = (
  import.meta.env.VITE_BACKEND_URL_GENERAL
    ? `${import.meta.env.VITE_BACKEND_URL_GENERAL}/documents`
...
const fetchPendingDocumentsByUnidad = async (unidadId) => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/unidad/${unidadId}/pending`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const fetchUserDocumentHistory = async (usuarioId) => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/usuario/${usuarioId}/historico`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const designarDocument = async (...args) => {
    console.log("Designar document called with args:", args);
  let idDocumento;
  let unidadDestino;
  let observaciones = "";
  let idUsuario = null; // revisar 

  if (args.length === 4) {
    [idUsuario, idDocumento, unidadDestino, observaciones] = args;
  } else if (args.length === 3) {
    [idDocumento, unidadDestino, observaciones] = args;
    const user = JSON.parse(localStorage.getItem("user"));
    idUsuario = user?.id || null;
  } else if (args.length === 2) {
    [idDocumento, unidadDestino] = args;
    const user = JSON.parse(localStorage.getItem("user"));
    idUsuario = user?.id || null;
  } else {
    return {
      success: false,
      error: "Parámetros insuficientes para designar documento",
    };
  }
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/${idDocumento}/designar`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const finalizarDocument = async (idDocumento, observaciones = "") => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/${idDocumento}/finalizar`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
const fetchUserDocumentStats = async (usuarioId) => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/usuario/${usuarioId}/estadisticas`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const fetchDerivacionesPendientes = async () => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/derivaciones/pendientes`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
const aprobarDerivacion = async (idDocumento) => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/${idDocumento}/aprobar-derivacion`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const rechazarDerivacion = async (idDocumento, razon) => {
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/${idDocumento}/rechazar-derivacion`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
const aceptarDocumento = async (idDocumento) => {
  const token = localStorage.getItem("token");
  try {
    const res = await fetch(`${BACKEND_URL}/${idDocumento}/aceptar`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
...
const fetchDocumentosPendientesAceptacion = async (idUnidad) => {
  const token = localStorage.getItem("token");
  try {
    const res = await fetch(
      `${BACKEND_URL}/unidad/${idUnidad}/pending-aceptacion`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    console.error(
      "Error fetching pending acceptance documents:",
      error,
    );
    return { success: false, error: "Error de conexión" };
  }
...
export {
  ...
  fetchUserDocumentHistory,
  fetchUserDocumentStats,
  designarDocument,
  finalizarDocument,
  fetchDocumentStats,
  fetchNotifications,
  fetchDerivacionesPendientes,
  aprobarDerivacion,
  rechazarDerivacion,
  aceptarDocumento,
